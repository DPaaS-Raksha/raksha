...
from raksha.openstack.common import lockutils
...
synchronized = lockutils.synchronized_with_prefix('raksha-')
...
def find_config(config_path):
    ...
    :raises: `raksha.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(FLAGS.state_path, "etc", "raksha", config_path),
        os.path.join(FLAGS.state_path, "etc", config_path),
        os.path.join(FLAGS.state_path, config_path),
        "/etc/raksha/%s" % config_path,
    ]
...
def execute(*cmd, **kwargs):
    ...
    corresponding filter to etc/raksha/rootwrap.d !
...
def rakshadir():
    import raksha
    return os.path.abspath(raksha.__file__).split('raksha/__init__.py')[0]
...
def monkey_patch():
    ...
    Example: 'raksha.api.ec2.cloud:' \
     raksha.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See raksha.openstack.common.notifier.api.notify_decorator)
...
def generate_mac_address():
    ...
    #             properly: 0xfa.
    #             Discussion: https://bugs.launchpad.net/raksha/+bug/921838
    mac = [0xfa, 0x16, 0x3e,
